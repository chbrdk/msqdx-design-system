...
const defaultExternalItems: AdminNavItem[] = [{ label: 'Settings', path: '/admin/settings', icon: 'settings' }];
...
const meta = {
    ...
    parameters: {
        ...
        docs: {
            description: {
                component: `
Seitennavigation für Admin-Bereiche: behoben (Desktop), Off-Canvas-Drawer (Mobile).
    ...
    render: (args) => {
        ...
        return (
            <Box sx={{ display: 'flex', height: '100vh', width: '100%' }}>
                <MsqdxAdminNav {...args} open={open} onClose={() => setOpen(false)} />
    ...
    render: (args) => {
        ...
        return (
            <Box sx={{ display: 'flex', height: '100vh', width: '100%' }}>
                ...
                />
                <Box sx={{ flex: 1, p: 3, bgcolor: 'background.default' }}>Theme-Toggle unten in der Nav.</Box>
            </Box>
        );
    },
};
